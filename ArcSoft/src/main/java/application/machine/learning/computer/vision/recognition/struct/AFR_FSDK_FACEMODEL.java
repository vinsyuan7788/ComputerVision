...
public class AFR_FSDK_FACEMODEL extends Structure {
    ...
    public AFR_FSDK_FACEMODEL deepCopy() throws Exception{
        ...
        feature.pbFeature.setPointer(C_Library.INSTANCE.malloc(feature.lFeatureSize));
        C_Library.INSTANCE.memcpy(feature.pbFeature.getPointer(),pbFeature.getPointer(),feature.lFeatureSize);
        return feature;
    }
    
    public synchronized void freeUnmanaged(){
        if(bAllocByMalloc&&isValid()){
            C_Library.INSTANCE.free(pbFeature.getPointer());
            pbFeature = null;
